...
export const ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="relative h-full z-20 flex-shrink-0 flex flex-col bg-white/60 backdrop-blur-xl border-l border-white/50 shadow-none overflow-hidden"
        ...
        >
          ...
          <div className="flex flex-col h-full w-[360px] pt-20 pb-32">
              ...
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/40 bg-white/20 shrink-0">
                ...
                <button 
                  onClick={onClose}
                  className="p-1.5 hover:bg-slate-100/50 rounded-full text-slate-400 hover:text-slate-600 transition-colors"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              {/* Input Area */}
              <form onSubmit={handleSend} className="p-3 bg-white/30 border-t border-white/40 shrink-0">
                <div className="flex items-center gap-2 bg-white/70 px-2 py-2 rounded-xl border border-white/60 focus-within:border-indigo-300 focus-within:ring-2 focus-within:ring-indigo-100 transition-all shadow-sm">
                  <input
                    ref={inputRef}
                    type="text"
                    value={input}
                  ...
                  <button
                    type="submit"
                    disabled={!input.trim()}
                    className="p-1.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg transition-all disabled:opacity-50 disabled:scale-95 active:scale-95 shadow-md shadow-indigo-200"
                  >
                    <Send size={14} />
                  </button>
                </div>
